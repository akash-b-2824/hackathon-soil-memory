...
def _memory_key(user_id: str, state: str, district: str) -> str:
    return f"{user_id.strip().lower()}::{state.strip().lower()}::{district.strip().lower()}"


def update_user_memory(user_id: str, state: str, district: str, crop: str) -> None:
    key = _memory_key(user_id, state, district)
    memory_store = STATE.setdefault("user_memory", {})
    history = memory_store.setdefault(key, [])
    history.append(crop.strip().lower())
    # Keep only the recent memory window for simple, stable behavior
    memory_store[key] = history[-6:]


def get_user_memory(user_id: str, state: str, district: str) -> List[str]:
    key = _memory_key(user_id, state, district)
    memory_store = STATE.setdefault("user_memory", {})
    return memory_store.get(key, [])


def suggest_next_crop_with_memory(payload: "SimulationInput") -> Dict[str, object]:
    ...
    history = get_user_memory(payload.user_id, payload.state, payload.district)
    last_crop = history[-1] if history else None
    prev_crop = history[-2] if len(history) >= 2 else None

    best_crop = None
    best_score = -1e9
    scored_options: List[Dict[str, float]] = []
    for i, crop_name in enumerate(classes):
        c = str(crop_name).strip().lower()
        base_conf = float(probs[i])
        soil_risk = get_soil_memory_score(payload.state, payload.district, c)

        # Penalize immediate repetition to promote rotation and long-term fertility
        repeat_penalty = 0.0
        if last_crop == c:
            repeat_penalty += 0.25
        if prev_crop == c:
        ...
        score = base_conf - (0.45 * soil_risk) - repeat_penalty
        scored_options.append({"crop": c, "score": float(score), "confidence": base_conf})
        if score > best_score:
            best_score = score
            best_crop = c

    top_options = sorted(scored_options, key=lambda x: x["score"], reverse=True)[:5]
    return {
        "suggested_next_crop": best_crop,
        "last_crop": last_crop,
...
def simulate(payload: SimulationInput) -> Dict[str, object]:
    results = simulate_crop_plan(payload)
    chosen_crop = results[0]["crop"] if results else ""
    if chosen_crop:
        update_user_memory(payload.user_id, payload.state, payload.district, chosen_crop)
    suggestion = suggest_next_crop_with_memory(payload)
    return {"results": results, "memory": suggestion}


@app.post("/api/next-crop")
def next_crop(payload: SimulationInput) -> Dict[str, object]:
    return suggest_next_crop_with_memory(payload)


@app.get("/api/memory/{user_id}/{state}/{district}")
def memory(user_id: str, state: str, district: str) -> Dict[str, object]:
    history = get_user_memory(user_id, state, district)
    return {
        "user_id": user_id,
        "state": state,
        "district": district,
...
<body>
  ...
  <div class='wrap'>
    ...
    <div id='simulator' class='tab'>
      <div class='card'>
        ...
        <div class='card-body'>
          <div class='grid'>
            <div class='field'><label for='user_id'>User ID (Memory Profile)</label><input id='user_id' placeholder='e.g. farmer_01' value='farmer_01' /></div>
          ...
          <div id='memoryBox' class='muted' style='margin-top:10px;'></div>
          <div id='results'></div>
        </div>
      </div>
    </div>
      ...
      <div class='card'>
        ...
        <div class='card-body'>
          ...
          <div class='link-list'>
            ...
            <a href='/api/next-crop' target='_blank'>Next Crop Suggestion (POST)</a>
  ...
  <script>
    ...
    async function runSim() {
      ...
      const hist = (memory.memory_history || []).join(' -> ') || 'No history yet';
      document.getElementById('memoryBox').innerHTML =
        `<strong>Memory Suggestion:</strong> Next best rotation crop is <strong>${memory.suggested_next_crop || '-'}</strong><br/>` +
        `<strong>Previous crops:</strong> ${hist}`;
    }
